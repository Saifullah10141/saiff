...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($faculty_id && $department_id) {
        ...
        $stmt = $conn->prepare("UPDATE course_coordinators SET user_id=?, father_name=?, cnic=?, dob=?, gender=?, faculty_id=?, department_id=? WHERE coordinator_id=?");
        ...
        if (($stmt->execute()) && ($stmt2->execute())) {
            ...
            $role = $_SESSION['role'];
            $ip = getUserIp();
            $action = "Updated course coordinator & Reg. No. : $id";
