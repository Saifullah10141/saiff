...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM course_coordinators WHERE coordinator_id = ?");
    ...
    if ($user_id) {
        ...
        $stmt1 = $conn->prepare("DELETE FROM course_coordinators WHERE coordinator_id = ?");
        ...
        $action = "Deleted course coordinator & Reg. No. : $reg_no";
