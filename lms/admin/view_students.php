...
<body>
                ...
                <div class="modal" id="editModal">
                    <div class="modal-content">
                        ...
                        <form>
                            ...
                            <label>Faculty</label>
                            <select name="faculty" id="faculty-select" onchange="loadDepartments()">
                                ${data.faculties.map(f => `<option value="${f.id}" ${studentData[7] === f.name ? 'selected' : ''}>${f.name}</option>`).join('')}
                            </select>
                            <label>Department</label>
                            <select name="department" id="department-select" onchange="loadSections()">
                                <!-- Department options will be loaded based on faculty selection -->
                            </select>
                            <label>Degree</label>
                            <select name="degree" id="degree-select" onchange="loadSemesters()">
                                ${data.degrees.map(dg => `<option value="${dg.id}" ${studentData[8] === dg.name ? 'selected' : ''}>${dg.name}</option>`).join('')}
                            </select>
    ...
    function loadDepartments() {
        const facultyId = document.querySelector('[name="faculty"]').value;
        fetch(`get_departments_by_faculty.php?faculty_id=${facultyId}`)
            .then(res => res.json())
            .then(data => {
                const departmentSelect = document.getElementById('department-select');
                departmentSelect.innerHTML = data.departments.map(d => `<option value="${d.department_id}">${d.name}</option>`).join('');
            });
    }
