...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $role = 'lms_manager';
     ...
     if (!$faculty_id || !$department_id) {
         echo "<script>alert('Invalid faculty or department selected.'); window.location.href='create_lms_manager.php';</script>";
         exit;
     }
    ...
    if (!isset($error)) {
        ...
        if (!$user_stmt) {
            ...
            if ($user_stmt->execute()) {
                ...
                $cc_stmt = $conn->prepare("INSERT INTO lms_managers (manager_id, user_id, father_name, cnic, dob, gender, faculty_id, department_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                if (!$cc_stmt) {
                    ...
                    if ($cc_stmt->execute()) {
                        ...
                        if ($activity_stmt) {
                            ...
                            $action = "Created LMS Manager ID & Reg. No. : $reg_no";
                        ...
                        header("Location: lms_manager_created.php");
                        exit();
                    }
                }
            }
...
if(!$flag){
    echo "<script>alert('LMS Manager ID already exists.');  window.location.href='create_lms_manager.php';</script>";
}
echo "<script>alert('Could not create LMS Manager ID.'); window.location.href='create_lms_manager.php';</script>";
?>
